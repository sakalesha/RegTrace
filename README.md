...
├── README.md
├── requirements.txt
├── vercel.json                          # Frontend static build config
...
│   │   ├── db/
│   │   │   └── mongodb.py               # Async MongoDB connection (Motor)
│   │   ├── api/
...
│   │   │       ├── dashboard.py
│   │   │       └── audit.py
│   │   ├── schemas/                     # Pydantic models (12 schema files)
│   │   │   ├── document.py              # DocumentStatus enum, DocumentOutput, etc.
│   │   │   ├── clause.py                # ClauseSchema, ClauseSegmentationInput
│   │   │   ├── obligation.py            # ObligationResponse, ObligationUpdate, BulkApproveRequest
│   │   │   ├── task.py                  # TaskResponse, TaskUpdate, TaskAssignRequest, enums
│   │   │   ├── evidence.py              # EvidenceResponse, EvidenceUpdate, EvidenceStatus
│   │   │   ├── compliance.py            # ComplianceOverview, ObligationCompliance, etc.
│   │   │   ├── gap.py                   # GapItem, GapOverview, GapType/GapSeverity enums
│   │   │   ├── report.py                # AuditReport, ReportSummary, ReportObligation
│   │   │   ├── search.py                # SearchResponse, SearchMode/SearchType enums
│   │   │   ├── review.py                # ReviewAction, ReviewListResponse
│   │   │   └── parsing.py               # ParsingInput, ParsingOutput, ParsedLine, ParsedPage
...
│   │   │   ├── task.py
│   │   │   └── evidence.py
│   │   ├── agents/                      # 6 AI agents (extend BaseAgent)
│   │   │   ├── base_agent.py            # Abstract base: validate → process → validate → persist
│   │   │   ├── ingestion_agent.py
│   │   │   ├── parsing_agent.py
...
│   │   │   ├── audit_service.py
│   │   │   └── job_registry.py          # Cooperative cancellation for background jobs
│   │   ├── utils/
│   │   │   ├── layout.py                # PyMuPDF block extraction + geometry
│   │   │   ├── storage.py               # Cloudinary upload/download
│   │   │   └── pipeline_log.py          # Structured pipeline event logging
│   │   └── tests/
│   │       ├── test_audit.py
│   │       ├── test_evidence.py
│   │       └── test_search_and_embedding.py
│   ├── scripts/                         # 7 standalone evaluation/test scripts
│   │   ├── eval_obligations.py
│   │   ├── eval_tasks.py
│   │   ├── eval_clauses.py
│   │   ├── eval_ingestion.py
...
│       ├── App.tsx
│       ├── lib/
│       │   ├── api.ts                   # Full API client (308 lines)
│       │   └── utils.ts
│       ├── components/
│       │   ├── layout/
│       │   │   ├── nav-items.ts         # Nav groups: Ingest, Analyze, Assurance
│       │   │   ├── sidebar.tsx
│       │   │   └── header.tsx
│       │   ├── dashboard/
│       │   ├── documents/
...
│   ├── wireframes/
│   └── Module-Wise-Design/
└── screenshots/                        # Drop screenshot images here
```
